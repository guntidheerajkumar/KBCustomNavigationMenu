...
@implementation KBDropdownView

#pragma mark - INIT

+ (instancetype)dropdownView
{
    return [[KBDropdownView alloc] init];
}
...
- (id)init
{
    ...
    if (self)
    {
        ...
        _direction = KBDropdownViewDirectionTop;
        _currentState = KBDropdownViewStateDidClose;
...
- (BOOL)isOpen
{
    return (_currentState == KBDropdownViewStateDidOpen);
}

- (void)showInView:(UIView *)containerView withContentView:(UIView *)contentView atOrigin:(CGPoint)origin
{
    if (_currentState != KBDropdownViewStateDidClose) {
        return;
    }
    
    // Start showing
    _currentState = KBDropdownViewStateWillOpen;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(self.animationDuration * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        _currentState = KBDropdownViewStateDidOpen;
...
- (void)hide
{
    if (_currentState != KBDropdownViewStateDidOpen) {
        return;
    }
    
    // Start hiding
    _currentState = KBDropdownViewStateWillClose;
...
- (void)setupContentView:(UIView *)contentView inView:(UIView *)containerView atOrigin:(CGPoint)origin
{
    ...
    switch (self.direction) {
        case KBDropdownViewDirectionTop:
            ...
            break;
        case KBDropdownViewDirectionBottom:
    ...
    if (self.direction == KBDropdownViewDirectionTop) {
...
- (void)addContentAnimationForState:(KBDropdownViewState)state
{
...
- (void)addContainerAnimationForState:(KBDropdownViewState)state
{
...
- (NSArray *)contentPositionValuesForState:(KBDropdownViewState)state
{
    ...
    if (state == KBDropdownViewStateWillOpen || state == KBDropdownViewStateDidOpen)
    {
        if (self.direction == KBDropdownViewDirectionTop) {
...
- (NSArray *)contentKeyTimesForState:(KBDropdownViewState)state
{
...
- (NSArray *)contentTimingFunctionsForState:(KBDropdownViewState)state
{
...
- (NSArray *)containerTransformValuesForState:(KBDropdownViewState)state
{
...
- (NSArray *)containerKeyTimesForState:(KBDropdownViewState)state
{
    NSMutableArray *keyTimes = [NSMutableArray new];
    [keyTimes addObject:[NSNumber numberWithFloat:0]];
    [keyTimes addObject:[NSNumber numberWithFloat:0.5]];
...
- (NSArray *)containerTimingFunctionsForState:(KBDropdownViewState)state
{
    NSMutableArray *timingFunctions = [NSMutableArray new];
    [timingFunctions addObject:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseOut]];
    [timingFunctions addObject:[CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseInEaseOut]];
